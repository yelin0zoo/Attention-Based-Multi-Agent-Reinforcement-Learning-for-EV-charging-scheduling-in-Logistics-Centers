"""
AttentionSAC (MAAC): 어텐션 기반 멀티에이전트 Soft Actor-Critic
중앙 어텐션 크리틱 + 분산 정책 네트워크 (CTDE)
"""
import torch
...
class AttentionSAC(object):
    ...
    def __init__(self, agent_init_params, sa_size,
                 ...
                 **kwargs):
        """
        reward_scale: SAC 온도 파라미터(α=1/reward_scale)의 역수. 클수록 결정적 정책 선호.
        """
    ...
    def update_critic(self, sample, soft=True, logger=None, **kwargs):
        """
        소프트 벨만 타겟: Q(s,a) ← r + γ · (Q_target(s', a') − α · log π(a'|s'))
        """
    ...
    def update_policies(self, sample, soft=True, logger=None, **kwargs):
        """
        SAC 정책 손실: L(π) = E[log π · (log π / α − A(s,a))], A = Q − V
        """
        obs, acs, rews, next_obs, dones = sample
        samp_acs = []
        all_probs = []
    ...
    def update_all_targets(self):
        """타겟 네트워크 소프트 업데이트: θ_target ← (1−τ)·θ_target + τ·θ"""
    ...
    def init_from_save(cls, filename, load_critic=False):
        """load_critic=True 시 크리틱/옵티마이저까지 복원 (학습 재개용)."""
