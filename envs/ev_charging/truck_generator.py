...
# 배터리 용량 (kWh) — eval_compare_10.py 등에서 시나리오별로 패치 가능
BATTERY_CAP_KWH = 60.0

# 모드별 λ 정의 (시간대 순서) — 10도크 기준
# 각 튜플: (minutes_end, normal_10, extreme, smooth, low)
# v2: 전체 × 0.649 스케일 (normal_10 최대 피크 1.54 → 1.00)
#     하루 기대 트럭: normal_10 ~65대, extreme ~69대, smooth ~64대, low ~32대
_RATE_TABLE = [
...
def get_arrival_rates(mode='normal'):
    ...
    모드 (v2: 전체 × 0.649 스케일):
      normal_10 - 10도크 기준 (일 ~65대, 최대 λ=1.00)
